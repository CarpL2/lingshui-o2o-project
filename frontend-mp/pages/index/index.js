...
export default {
	...
	methods: {
			...
			// 1. 确保索引归零
			    ...
			    getCategoryList().then((res) => {
			        if (res && res.code === 1) {
			            ...
			            // 正常加载第一个真实的菜品分类（比如“蜀味烤鱼”）
			            if (res.data.length > 0) {
			                this.getDishListDataes(res.data[0], 0)
			            }
			        }
			    })
		...
		goSeckill() {
		   // 模拟数据：这里填你想要展示的秒杀信息
		       // ⚠️重要：id 必须是你数据库里 seckill_dish 表里真实存在的 ID，否则抢不到
		       let id = 1; 
		       let name = "至尊酸菜鱼";
		       let price = 9.9; // 你想定的价格
		       let oldPrice = 88.0;
		   
		       // 关键点：把这些信息拼在 URL 后面传给下一个页面
		       uni.navigateTo({
		           url: `/pages/seckill/index?id=${id}&name=${name}&price=${price}&oldPrice=${oldPrice}`
		       });
		},
		...
		async goSeckill() {
		    ...
		    if (res.code === 1 && res.data && res.data.length > 0) {
		      // 3. 取出第一个活动的数据
		      const firstActivity = res.data[0]
		      const seckillId = firstActivity.id
		      
		      // 4. 带着真 ID 跳到详情页！
		      uni.navigateTo({
		        url: `/pages/seckill/index?id=${seckillId}`
		      })
		    } else {
		      uni.showToast({
		        title: '暂无秒杀活动',
		...
		async getMerchantInfo() {
			return new Promise((resolve, reject) => {
				getMerchantInfo().then(res => {
					...
					// --- 关键修改在这里 ---
					// 如果后端返回的是 {code:1, data:{...}} 这种格式，我们要取里面的 data
					const realData = res.data ? res.data : res
					
					if (this.$store) {
						this.$store.commit('setShopInfo', realData)
						// 兼容 deliveryPrice 和 deliveryFee
						let fee = realData.deliveryPrice !== undefined ? realData.deliveryPrice : (realData.deliveryFee || 0)
						this.$store.commit('setDeliveryFee', fee)
					}
					
					// 存入本地变量 (给页面直接用)
					this.shopInfoData = realData
