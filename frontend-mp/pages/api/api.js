...
// 秒杀抢购接口
export const grabSeckill = (dataObj) => {
    return request({
        // 🚀 核心修改：把 ID 直接拼在 URL 路径里！
        // 例如发送到：/user/seckill/grab/1
        url: `/user/seckill/grab/${dataObj.seckillId}`,
        method: 'POST'
    })
}
...
export const getSeckillResult = (seckillId) => {
  return request({
    url: `/user/seckill/result/${seckillId}`,
    method: 'get'
  })
}

// 查询历史记录
export const getSeckillHistory = () => {
  return request({
    url: `/user/seckill/history`,
    method: 'get'
  })
}
