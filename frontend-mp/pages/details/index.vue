    ...
    <view class="order_content orderDetail">
      <view class="order_content_box" scroll-y="true" scroll-top="0rpx">
        ...
        <status ref="status" :timeout="timeout" :orderDetailsData="orderDetailsData" :rocallTime="rocallTime"
		  ...
		  @handleCancel="handleCancel"> </status>
        <!-- end -->
		<view v-if="orderDetailsData.status === 6" class="cancel_reason_box">
		  <view class="reason_title">取消原因</view>
		  <view class="reason_content">
		    {{ orderDetailsData.rejectionReason || orderDetailsData.cancelReason || '用户已取消' }}
		  </view>
		</view>
        <!-- 订单详情 -->
...
<style src="../order/style.scss" lang="scss">
</style>
