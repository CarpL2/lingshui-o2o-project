CREATE DATABASE  IF NOT EXISTS `lingshuio2o` ;
USE `lingshuio2o`;

DROP TABLE IF EXISTS `seckill_dish`;
CREATE TABLE `seckill_dish` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键',
  `dish_id` bigint NOT NULL COMMENT '关联的原菜品ID (对应 dish 表的 id)',
  `title` varchar(50) DEFAULT NULL COMMENT '秒杀活动标题 (例如: 1分钱抢特供红烧肉)',
  `seckill_price` decimal(10,2) NOT NULL COMMENT '秒杀价格 (比如 0.01)',
  `stock_count` int NOT NULL COMMENT '秒杀库存数量',
...
CREATE TABLE `seckill_voucher` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键',
  `user_id` bigint NOT NULL COMMENT '抢购的用户ID',
  `seckill_dish_id` bigint NOT NULL COMMENT '关联的秒杀活动ID',
  `voucher_code` varchar(6) NOT NULL COMMENT '6位数字核销码 (用户凭此码到店)',
  `status` int DEFAULT '0' COMMENT '核销状态 0:待核销 1:已核销',
  `order_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '抢购时间',
  `use_time` datetime DEFAULT NULL COMMENT '实际核销时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_code` (`voucher_code`) -- 保证核销码唯一，方便索引
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='秒杀核销券明细表';
ALTER TABLE seckill_voucher
ADD UNIQUE INDEX unq_user_seckill (user_id, seckill_dish_id);
