        ...
        <div>
          <el-form-item label="套餐图片:"
            ...
            <image-upload :prop-image-url="imageUrl"
              ...
              图片大小不超过2M<br>仅能上传 PNG JPEG JPG JFIF类型图片<br>建议上传200*200或300*300尺寸的图片
            </image-upload>
          </el-form-item>
        </div>
        <!-- <el-form-item
...
export default class extends Vue {
  ...
  public async submitForm(formName: any, st: any) {
    ...
    form.validate(async (valid: any) => {
      ...
      // 4. 发起请求 (使用 try-catch 替代嵌套的 .then)
      try {
        let res: any;
        if (this.actionType == 'add') {
          delete prams.id;
          res = await addSetmeal(prams);
        } else {
          delete prams.updateTime;
          res = await editSetmeal(prams);
        }

        // 5. 统一处理结果
        if (res && res.data && res.data.code === 1) {
          this.$message.success(this.actionType === 'add' ? '套餐添加成功！' : '套餐修改成功！');
          ...
          if (!st) {
            ...
            this.ruleForm = {
              ...
              description: '', dishList: [], status: true, id: '', idType: ''
            } as any;
          }
        } else {
        ...
        // 关键：拦截器导致 Promise 链断裂会进入这里，但我们可以静默处理已成功的逻辑
        console.log('捕获到非核心异常:', err);
        // 如果数据已经修改成功（通过后端确认），此处不再弹窗报错
      }
    });
  }
