...
export default class extends Vue {
  ...
  private async submitForm(formName: any, st: any) {
    // 1. 获取表单引用并校验
    const form = this.$refs[formName] as any;
    form.validate(async (valid: boolean) => {
      if (!valid) return; // 校验不通过直接结束

      if (!this.ruleForm.image) {
        this.$message.error('菜品图片不能为空');
        return;
      }
      ...
      try {
        ...
        if (this.actionType === 'add') {
          ...
          res = await editDish(params);
        }

        // 3. 处理结果 (注意：不再使用嵌套的 .then)
        if (res && res.data && res.data.code === 1) {
          this.$message.success(this.actionType === 'add' ? '菜品添加成功！' : '菜品修改成功！');
          ...
          if (!st) {
            ...
            this.ruleForm = {
              ...
              description: '', dishFlavors: [], status: true, categoryId: ''
            };
            this.restKey++;
          }
        } else {
        ...
        // 如果数据其实改成功了，这里可以不给用户弹强烈的报错提示，或者只弹一个简单的
        if (err.message && err.message.includes('undefined')) {
            // 忽略由于组件销毁导致的 undefined 错误
            return;
        }
      }
    });
